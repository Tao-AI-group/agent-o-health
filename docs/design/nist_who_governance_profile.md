...
Milestone 4 represents governance frameworks as profiles, SKOS concept schemes, competency queries, and SHACL constraints. The intent is not to import NIST AI RMF or WHO health AI guidance as strict ontologies, but to make their expectations operational for agent-paper annotation and reporting completeness assessment.
...
| Map | `health:ClinicalSetting`, `report:IntendedUse`, `health:PatientPopulation`, `health:HealthDataSource`, `gov:Risk` |
...
- `data/examples/governance_reporting_profile_example.ttl` demonstrates a governed clinical decision-support agent and a reporting-completeness POC case.
